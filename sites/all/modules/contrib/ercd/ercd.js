...
(function($) {
    ...
    attach : function(context, settings) {
      ...
      $('.ercd-trigger').change(function() {
        if ($(this).attr('checked')) {
          ...
          $(this).parent().next('.form-type-radios').find('.ercd-default-state').fadeIn();
        } else {
          $(this).parent().next('.ercd-child').css('display', 'none');
          $(this).parent().next('.form-type-radios').find('.ercd-default-state').css('display', 'none');
        }
      });

      // Toggle checkboxes and adjust radios on the settings page.
      $('.ercd-setting').change(function() {
        var checked = $(this).attr('checked');

        // Update all child checkboxes and radios if not checked.
        if (!checked) {
          // Get child container.
          ...
          $(this).parent().next('.form-type-radios').find('.form-radio').each(function() {
      ...
      $('.ercd-option').change(function() {
        ...
        if ($ercdContainer.length) {
          ...
          // Update all child checkboxes if unchecked. This will force users
          // manually drill down if the default state is unselected.
          if (!checked) {
            ercd_toggle_checkboxes($ercdContainer, checked);
          }
        }
  ...
  function ercd_toggle_checkboxes($container, toggleValue) {
    ...
    if ($container.attr('tagName').toLowerCase() != 'fieldset') {
      if (toggleValue) {
        // Update the next fieldset and set the correct user message.
        $fieldsets = $container.children('fieldset');
        $fieldset = $($fieldsets[0]);
        var ercdUserRole = $fieldset.hasClass('ercd-user-roles');
        if (!(ercdUserRole && toggleValue)) {
          if (ercdUserRole) {
            $fieldset.children('.fieldset-wrapper').children('.form-checkboxes').children('.form-type-checkbox').children('.form-checkbox').attr('checked', toggleValue);
          } else {
            // Set the option checkbox.
            $fieldset.children('.ercd-child').children('.form-checkbox').attr('checked', toggleValue);
            $fieldset.children('.fieldset-wrapper').children('.form-type-checkbox').children('.form-checkbox').attr('checked', toggleValue);
          }
        }
        ercd_set_options_state($fieldset);
      } else {
        ...
        $($fieldsets.get().reverse()).each(function() {
          ...
          if (!(ercdUserRole && toggleValue)) {
            if (ercdUserRole) {
              ...
              // Set the option checkbox.
              if (toggleValue) {
                $(this).children('.ercd-child').children('.form-checkbox').attr('checked', toggleValue);
                $(this).children('.fieldset-wrapper').children('.form-type-checkbox').children('.form-checkbox').attr('checked', toggleValue);
              } else {
                $(this).find('.form-checkbox').attr('checked', toggleValue);
              }
            }
          }
          ercd_set_options_state($(this));
  ...
  function ercd_set_options_state($container) {
    ...
    $checkboxes.each(function() {
      ...
      if ($(this).attr('checked')) {
        ...
        var $radios = $fieldset.find('.ercd-default-state');
